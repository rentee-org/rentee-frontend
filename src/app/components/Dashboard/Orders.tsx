...
import {
    ...
    const filteredData = mockData.filter((listing) =>
        listing.renter.name.toLowerCase().includes(searchQuery.toLowerCase()),
    )
    ...
    return (
        ...
        <div className={showDetails ? "transition-all duration-300" : "transition-all duration-300"}>
            <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-6 bg-gray">
                    ...
                    <TableBody>
                        ...
                        <TableRow key={listing.id} className="hover:bg-gray-50">
                            ...
                            <div className="flex items-center space-x-3">
                                {listing.renter.avatar ? (
                                <img
                                    src={listing.renter.avatar || "/placeholder.svg"}
                                    alt={listing.renter.name}
                                    className="w-10 h-10 rounded-full object-cover"
                                />
                                ) : (
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-medium text-sm ${listing.renter.avatarColor}`}
                                >
                                    {listing.renter.initials}
                                </div>
                                )}
                                <span className="font-medium text-gray-900">{listing.renter.name}</span>
                            </div>
                            </TableCell>
        ...
        {showDetails && selectedListing && (
            <div className="fixed right-0 top-0 h-[100vh] p-4 w-full sm:w-[90%] md:w-[450px] lg:w-[500px] bg-white shadow-xl z-50 transition-all duration-300">
                ...
                <div className="flex items-center space-x-3 mb-6">
                    {selectedListing.renter.avatar ? (
                    <img
                        src={selectedListing.renter.avatar || "/placeholder.svg"}
                        alt={selectedListing.renter.name}
                        className="w-12 h-12 rounded-full object-cover"
                    />
                    ) : (
                    <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-medium ${selectedListing.renter.avatarColor}`}
                    >
                        {selectedListing.renter.initials}
                    </div>
                    )}
